...
const cd = new Set() //New set for the cooldown
const cdTime = 5000 //If you want to, you can change the cooldown time (milliseconds)
...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.isCommand()) {
      ...
      if (cd.has(interaction.user.id)) {
        ...
        await interaction.reply({
          embeds: [
            {
              author: {
                name: `Охолодись! Команди можна писати лишень раз в ${cdTime / 1000} секунд!`,
              },
              color: 0xcc7229,
            },
          ],
      ...
      //If interaction is context command
      const { commands } = interaction.client //Get commands from client
      const { commandName } = interaction //Get command name from interaction
      const contextCommand = commands.get(commandName) //Find command by command name
      ...
      if (button.permission) {
        //If button file has permissions
        let permissionBit = new PermissionsBitField([button.permission]) //Make a permissions bit
        if (!interaction.member.permissions.has(permissionBit))
          return await interaction.reply({
            embeds: [
              {
                author: { name: 'Недостатньо прав!' },
                color: 0xcc7229,
              },
            ],
            ephemeral: true,
          }) //Return if user doesn't have specified permissions
      }
      ...
      if (interaction.customId === 'roles') {
        ...
        if (!prefix) {
          ...
          return interaction.reply({
            content: 'На цьому сервері більше не працюють ролі для видачі',
            ephemeral: true,
          })
        }
        ...
        if (max && interaction.message.components[0].components[0].data.max_values !== max) {
          await interaction.reply({
            content: 'Цей список не відповідає дійсності, ми оновили його.',
            ephemeral: true,
          })
        ...
        //So I through that 'manageable' means that it shows whenever bot can edit user's roles. (?)
        //I'll remove it and see how it'll perform

        if (!interaction.guild.members.me.permissions.has(PermissionFlagsBits.ManageRoles))
          return await interaction.reply({
            content: 'Я не маю відповідних прав на виконання цієї дії',
            ephemeral: true,
          })

        for (let i in list) {
          let role = interaction.guild.roles.cache.get(list[i].value)
          if (!role || !role.name.startsWith(prefix))
            return await interaction.reply({
              content: 'Помилка. Спробуйте вивести список ще раз',
              ephemeral: true,
            })
        ...
        const embed = new EmbedBuilder().setAuthor({ name: 'Ваші ролі оновлено!' }).setColor('Random')
