...
module.exports = {
  ...
  async execute(oldVoiceState, newVoiceState, client) {
    ...
    if (await db.get(cur.guild.id)) {
      //Require lobby channel from db, if it's exist
      if (cur.channel && cur.channel.id === (await db.get(cur.guild.id))) {
        //If current channel id = lobby channel
        ...
        if (set) {
          //If user already has a private channel
          if (channel) {
            cur.setChannel(channel).catch((err) => {
              console.error(err)
            }) //If channel - set user's voice channel to his private channel
          }
        } else {
