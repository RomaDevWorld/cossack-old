  ...
  async execute(interaction) {
    if (cd.has(interaction.user.id))
      return await interaction.reply({
        content: 'Тікети можна створювати раз в 10 хвилин',
        ephemeral: true,
      })
    ...
    if (!interaction.message.embeds[0].data.footer) {
      ...
      return interaction.reply({
        content: 'Це стара версія тікетів. Створіть це повідомлення знову командою /ticket',
        ephemeral: true,
      })
    }
    ...
    try {
      ...
      const embed = new EmbedBuilder().setAuthor({ name: 'Тікет створено!' }).setDescription('**Незабаром уповноважені люди зможуть відповісти на ваше запитання.**\nВи можете почати описувати свою проблему поки очікуєте.').setColor('Green')

      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`tick_0`).setLabel('Закрити обговорення').setStyle(ButtonStyle.Danger))

      channel.send({ content: '@here', embeds: [embed], components: [row] }).then((msg) => msg.pin())

      await interaction.reply({
        content: `Тікет створено! Продовжуйте в ${channel}`,
        ephemeral: true,
      })
