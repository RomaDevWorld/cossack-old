  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommandGroup((group) =>
      group
        ...
        .addSubcommand((subcommand) => subcommand.setName(`delete`).setDescription(`Видалити Ваш особистий канал.`))
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName(`invite`)
    ...
    .addSubcommand((subcommand) => subcommand.setName(`create`).setDescription(`Створити/Пере-створити Ваш особистий канал.`)),
  async execute(interaction) {
    ...
    if (interaction.options.getSubcommand() === `create`) {
      ...
      if (!(await db.get(interaction.guild.id)) || !interaction.guild.channels.cache.get(await db.get(interaction.guild.id))) {
        //Looking for lobby channel in database
        await db.delete(interaction.guild.id) //Delete lobby channel from db if it's set but isn't on a server
        let embed = new EmbedBuilder().setAuthor({ name: `Відсутній канал лобі!` }).setColor('Orange').setDescription('На цьому сервері відсутній канал лобі.\nЗверніться до адміністратора.').setFooter({ text: `Встановити: /prefs vclobby [lobby]` })
        return await interaction.reply({
          embeds: [embed],
          ephemeral: true,
        }) //Creates an embed then sends it
      }
    ...
    if (!channels[`${interaction.guild.id}_${interaction.member.id}`] || !channel)
      return await interaction.reply({
        embeds: [
          {
            author: { name: `У вас немає створеного каналу!` },
            color: 0xcc7229,
          },
        ],
        ephemeral: true,
    ...
    if (interaction.options.getSubcommand() === `invite`) {
      ...
      await interaction.reply({
        embeds: [
          {
            author: {
              name: `${member.user.username} був добавлений до Вашого каналу!`,
            },
            color: 0x2cf5aa,
          },
        ],
    ...
    if (interaction.options.getSubcommand() === `kick`) {
      ...
      await interaction.reply({
        embeds: [
          {
            author: {
              name: `${member.user.username} був виключений з Вашого каналу!`,
            },
            color: 0x2cf5aa,
          },
        ],
      ...
      if (interaction.options.getSubcommand() === `public`) {
        ...
        if (bool) {
          ...
          channel.permissionOverwrites.edit(interaction.guild.id, {
            ViewChannel: true,
          }) //Allow all the members to view user's private channel
          return await interaction.reply({
            embeds: [
              {
                author: {
                  name: `Ваш канал тепер доступний усім!`,
                },
                color: 0x2cf5aa,
              },
            ],
          ...
          channel.permissionOverwrites.edit(interaction.guild.id, {
            ViewChannel: false,
          }) //Deny all the member to view user's private channel
          return await interaction.reply({
            embeds: [
              {
                author: {
                  name: `Ваш канал тепер доступний тільки Вам, та усім кого Ви запросили.`,
                },
                color: 0x2cf5aa,
              },
            ],
        ...
        var limit
        if (num > 99) limit = 99 //If integer is more than 99 = limit-99
        else if (num < 2) limit = 2 //If integer is less than 2 = limit-2
        else limit = num //Integer is limit
        channel.setUserLimit(limit) //Set channel's user limit to limit
        return await interaction.reply({
          embeds: [
            {
              author: { name: `Ліміт учасників - ${limit}` },
              color: 0x2cf5aa,
            },
          ],
          ephemeral: true,
