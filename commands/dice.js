...
module.exports = {
  data: new SlashCommandBuilder().setName('dice').setDescription('Створити стіл для гральних кубиків'),
  async execute(interaction) {
    let embed = new EmbedBuilder().setAuthor({ name: 'Стіл для гральних кубиків!' }).setFooter({ text: 'Натисніть на кнопку щоб кинути кубик' }).setColor('White')

    let random = Math.floor(Math.random() * 10000000)
    let row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(random.toString()).setLabel('Кинути кубики').setStyle(ButtonStyle.Primary).setEmoji('🎲'))
    ...
    collector.on('end', async (collected) => {
      if (Object.keys(members).length === 0) {
        let newEmbed = new EmbedBuilder().setAuthor({ name: 'Стіл для гральних кубиків!' }).setFooter({ text: 'Ніхто не кинув кубик' }).setColor('Red')
