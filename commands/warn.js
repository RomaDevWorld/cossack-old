...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('list')
        .setDescription('Список попереджень конкретного учасника')
        .addUserOption((option) => option.setName('member').setDescription('Учасник').setRequired(true))
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName('add')
        .setDescription('Відправити попередження учаснику')
        .addUserOption((option) => option.setName('member').setDescription('Учасник').setRequired(true))
    ...
    .addSubcommand((subcommand) =>
      subcommand
        .setName('clear')
        .setDescription('Очистити список попереджень учасника')
        .addUserOption((option) => option.setName('member').setDescription('Учасник').setRequired(true))
        .addStringOption((option) => option.setName('reason').setDescription('Коментар до очищення'))
    )
    .setDescription('Попередження учасника про порушення')
  ...
  async execute(interaction) {
    ...
    if (!member)
      return await interaction.reply({
        content: 'Цього користувача немає на цьому сервері',
        ephemeral: true,
      })
    if (member.user.bot)
      return await interaction.reply({
        content: 'Цю команду не можна використовувати на ботів',
        ephemeral: true,
      })
    ...
    if (interaction.options.getSubcommand() === 'list') {
      ...
      let embed = new EmbedBuilder()
        .setAuthor({
          name: `Список попереджень учасника ${member.user.username}`,
          iconURL: member.user.avatarURL({ dynamic: true }),
        })
        .setColor('Orange')
        .setFooter({
          text: `Загальна кількість: ${fdb.length || '0'}`,
      ...
      if (member.roles.highest.position > interaction.member.roles.highest.position)
        return await interaction.reply({
          content: 'Цей користувач знаходиться вище за Вас в правах.',
          ephemeral: true,
        })

      await db.push(`${interaction.guild.id}.${member.id}`, {
        time: Date.now(),
      ...
      try {
        let dmEmbed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: 'Що це означає?',
              value: 'Особисто ми не вживаємо ніяких заходів.\nПри великій кількості попереджень модератори серверу можуть накласти на Вас обмеження на свій розсуд.',
            },
            {
              name: 'Ваша кількість попереджень за весь час',
              value: `${fdb.length + 1}`,
            }
          )
          .setFooter({
            text: 'Попередження з серверу ' + interaction.guild.name,
            iconURL: interaction.guild.iconURL(),
          })
      ...
      let logEmbed = new EmbedBuilder()
        .setAuthor({
          name: `Нове попередження | ${member.nickname || member.user.username}`,
          iconURL: member.user.avatarURL({ dynamic: true }),
        })
        .setColor('Orange')
        .setTimestamp()
        ...
        .addFields(
          { name: 'Учасник', value: `${member}`, inline: true },
          {
            name: 'Модератор',
          ...
          {
            name: 'Причина',
            value: `${reason.slice(0, 1000) || 'Причина не вказана'}`,
          }
        )
      ...
      let logEmbed = new EmbedBuilder()
        .setAuthor({
          name: `Попередження очищені | ${member.nickname || member.user.username}`,
          iconURL: member.user.avatarURL({ dynamic: true }),
        })
        .setColor('Green')
        .setTimestamp()
        ...
        .addFields(
          ...
          {
            name: 'Коментар',
            value: `${reason.slice(0, 1000) || 'Не вказано'}`,
          }
        )
      channel.send({ embeds: [logEmbed] }).catch((err) => console.error(err))
    }
