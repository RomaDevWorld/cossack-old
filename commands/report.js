const { ContextMenuCommandBuilder, ApplicationCommandType, EmbedBuilder, ActionRowBuilder, ButtonBuilder } = require('discord.js')
  ...
  async execute(interaction) {
    ...
    if (message.author.bot)
      return await interaction.reply({
        embeds: [
          {
            author: { name: 'Не можна скаржитись на ботів!' },
            color: 0xcc2929,
          },
        ],
        ephemeral: true,
    ...
    if (message.author.id === interaction.user.id)
      return await interaction.reply({
        embeds: [
          {
            author: { name: 'Не можна скаржитись на себе!' },
            color: 0xcc2929,
          },
        ],
        ephemeral: true,
      }) //User can't report itself

    if (!count[message.author.id]) {
      //If user was not reported last minute
      count[message.author.id] = {
        //Add an author to an array
        members: [interaction.user.id], //Push interaction.user's id to an array
      }
      setTimeout(() => {
        delete count[message.author.id]
      }, 60000) //create timeout
    } else {
      if (count[message.author.id].members.includes(interaction.user.id))
        return await interaction.reply({
          embeds: [
            {
              author: {
                name: 'Ви вже поскаржились на цього учасника!',
              },
              color: 0xcc2929,
            },
          ],
          ephemeral: true,
        }) //Return if user already reported this author
      ...
      if (log) {
        //If log channel exist
        const row = new ActionRowBuilder() //Make an action row with 3 buttons (ban, kick, mute)
          .addComponents(new ButtonBuilder().setCustomId(`mute_${message.author.id}`).setLabel('Заблокувати чат (5 хв)').setStyle(`Danger`))
          .addComponents(new ButtonBuilder().setCustomId(`kick_${message.author.id}`).setLabel('Вигнати').setStyle(`Danger`))
          .addComponents(new ButtonBuilder().setCustomId(`ban_${message.author.id}`).setLabel('Заблокувати (Очистити повідомлення)').setStyle(`Danger`))
        let embed = new EmbedBuilder() //Make the new embed
          .setAuthor({
            name: `Нова скарга! (${count[message.author.id].members.length}/${threshold})`,
          }) //Add an author to an embed
          .addFields({ name: `Учасник`, value: `${message.author}` }) //Add a field with the author's name

        if ((count[message.author.id].members.length / threshold) * 100 === 100) {
          //if user has been reported enough times
          embed //Add color, desc, footer to the embed
            .setColor(`Red`)
            .setDescription('Контекст повідомлення\n```' + message.content + '```')
            .setFooter({
              text: `Ми видалили повідомлення та заблокували чат на 60 секунд`,
            })
          message.delete()
          ...
          message.member
            ...
            .catch(() => {}) //Try to send a message to the author

          log.send({ embeds: [embed], components: [row] }) //Send embed to the log channel
        } else if ((count[message.author.id].members.length / threshold) * 100 > 50) {
          //if user has been reported half of the times
          embed
            ...
            .setDescription('Контекст повідомлення\n```' + message.content + '```')
            .setFooter({
              text: 'Ми не вжили ніяких заходів, поки-що..',
            })

          log.send({ embeds: [embed], components: [row] }) //Send embed to the log channel
        } else {
          //if user has been reported less than half of the times
          embed
            .setTitle(`ПЕРЕЙТИ ДО ПОВІДОМЛЕННЯ`)
            .setURL(message.url)
            .setColor(`Aqua`)
    ...
    await interaction.reply({
      embeds: [
        {
          author: { name: 'Скарга успішно відправлена!' },
          color: 0x33a64e,
        },
      ],
      ephemeral: true,
