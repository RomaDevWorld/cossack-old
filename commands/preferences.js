...
const exist = require('../functions/multiValue.js')
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommandGroup((group) =>
      group
        ...
        .addSubcommand((subcommand) => subcommand.setName(`switch`).setDescription(`Перемикачі типів подій`))
    )
    .addSubcommandGroup((group) =>
      group
        .setName(`counter`)
        ...
        .addSubcommand((subcommand) => subcommand.setName(`remove`).setDescription(`Видалити лічильник`))
    )
    .addSubcommandGroup((group) =>
      group
        .setName(`roles`)
        .setDescription("Авто-роль при заході та відновлення ролей при пере-під'єднанні")
        .addSubcommand((subcommand) =>
          subcommand
            .setName(`autorole`)
            .setDescription(`Увімкнути/Вимкнути автоматичну видачу вказаної ролі при приєднанні нового учасника`)
            .addRoleOption((option) => option.setName(`role`).setDescription(`Роль, що буде видана при приєднані нового учасника`))
        )
        .addSubcommand((subcommand) =>
          subcommand
            .setName(`restore`)
            .setDescription(`Відновлювати ролі та нікнейм при пере-під'єднанні учасника`)
            .addBooleanOption((option) => option.setName('bool').setDescription('Увімкнути або Вимкнути функцію').setRequired(true))
        )
    ),
  async execute(interaction) {
    if (!interaction.options.getSubcommandGroup()) {
      //If interaction has no command group
      if (interaction.options.getSubcommand() === `vclobby`) {
        //If subcommand's name is 'vclobby'
        ...
        if (!channel) {
          ...
          let embed = new EmbedBuilder()
            .setAuthor({
              name: `Лобі видалено!`,
              url: require(`../functions/memes.js`)(1),
            })
            .setColor('Orange')
      ...
      if (interaction.options.getSubcommand() === `colors`) {
        ...
        let embed = new EmbedBuilder()
          ...
          .setColor('Green')
          .setFooter({
            text: 'Створіть ролі, назва яких починається з встановленого префіксу',
          })
    ...
    if (interaction.options.getSubcommandGroup() === `counter`) {
      ...
      if (interaction.options.getSubcommand() === `remove`) {
        ...
        await counters.set(`${interaction.guild.id}`, {
          id: channel.id,
          name: interaction.options.getString(`name`) || `Учасники: ON/ALL`,
        }) //Sets a value at the db

        let embed = new EmbedBuilder()
          .setColor(`Green`)
          .setAuthor({
            name: `Лічильник встановлено!`,
            url: memes(1),
          })
          .addFields(
            {
          ...
          .setFooter({
            text: `Лічильник оновлюється кожні 5 хвилин (Обмеження API)`,
          })
    ...
    if (interaction.options.getSubcommandGroup() === `log`) {
      ...
      if (interaction.options.getSubcommand() === 'switch') {
        ...
        //Defines if type is on or off, sets a 'Success' style if on
        else msg.setStyle(ButtonStyle.Danger) //Sets a 'Danger' style if off

        const mem = new ButtonBuilder().setCustomId('sw_members').setLabel('Учасники')
        if (exist(sws, types['sw_members'])) mem.setStyle(ButtonStyle.Success) //Does the same thing
        else mem.setStyle(ButtonStyle.Danger)

        const voi = new ButtonBuilder().setCustomId('sw_voices').setLabel('Голосові канали')
        ...
        const embed = new EmbedBuilder()
          .setAuthor({
            name: `Перемикачі типів подій!`,
            iconURL: interaction.guild.iconURL(),
          })
          .setDescription(`Зелений колір кнопки [🟩] - подія **увімкнена**.\nЧервоний колір кнопки [🟥] - подія **вимкнена**.\n`)
          .addFields([
            {
              name: `Канал для подій`,
              value: `${interaction.guild.channels.cache.get(log) || `Встановіть його через **/prefs logs channel**`}`,
            }, //Adds a field if channel was found, if not - hint how to set it
          ])
          .setFooter({
            text: `Натискання на кнопку увімкне або вимкне тип події`,
          })
          .setColor(`White`)
        await interaction.reply({
          ...
          components: [row],
          ephemeral: true,
        }) //Sends an embed with all the buttons
      }
    }
      ...
      if (interaction.options.getSubcommand() === 'autorole') {
        ...
        if (!role) {
          ...
          return await interaction.reply({
            embeds: [
              {
                description: `**Ми не будемо видавати ролі при заході на сервер**`,
                color: 0x33a64e,
              },
            ],
            ephemeral: true,
        ...
        if (role.position > interaction.member.roles.highest.position)
          return await interaction.reply({
            embeds: [
              {
                description: `**Ви не можете встановити роль, яку не можете собі видати**`,
                color: 0xcc2929,
              },
            ],
            ephemeral: true,
        ...
        return await interaction.reply({
          embeds: [
            {
              description: `**Ми будемо видавати роль ${role} учасникам що зайдуть на сервер**`,
              color: 0x33a64e,
            },
          ],
          ephemeral: true,
        ...
        if (bool) {
          ...
          return await interaction.reply({
            embeds: [
              {
                author: {
                  name: 'Тепер ми будемо відновлювати усі ролі та нікнейм учасника що пере-зайшов',
                },
                color: 0x33a64e,
              },
            ],
          ...
          return await interaction.reply({
            embeds: [
              {
                author: {
                  name: 'Ми більше не будем відновлювати ролі та нікнейми учасників що перезайшли',
                },
                color: 0x33a64e,
              },
            ],
