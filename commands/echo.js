...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('echo')
    .setDescription('Конструктор повідомлень.')
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption((option) => option.setName('text').setDescription('Текст повідомлення'))
    .addStringOption((option) => option.setName('author').setDescription('Вибрати автора повідомлення').addChoices({ name: 'Бот', value: 'bot' }, { name: 'Сервер (Webhook)', value: 'server' }))
  ...
  async execute(interaction) {
    ...
    if (options._hoistedOptions.length === 0)
      return await interaction.reply({
        content: 'Вам необхідно вказати одну з опцій!',
        ephemeral: true,
      })
    ...
    await interaction.reply({
      embeds: [
        {
          description: '**Формуємо попередній перегляд.. Це може зайняти трохи часу** \n\nЦікавий факт, на випадок якщо це триває достатньо довго:\n' + require('../functions/memes.js')(3),
        },
      ],
      ephemeral: true,
    })
    ...
    //Embed author
    if (embedAuthor && embedAuthor.length > 256)
      return interaction.editReply({
        embeds: [
          {
            author: {
              name: 'Максимальна кількість символів embed.author = 256',
            },
            color: 0xeb4c34,
          },
        ],
        ephemeral: true,
        components: [],
      }) //Return error if specified embed author is too long
    if (embedAuthor && embedAuthorIcon && embedAuthorIcon.contentType.startsWith('image')) {
      //Check if author is specified. If author icon also was specified, and it's an image, put it as author icon
      embed.setAuthor({ name: embedAuthor, iconURL: embedAuthorIcon.url })
    } else {
      //If image is not correct, just set an author
      ...
      if (embedDesc.length > 4096)
        return interaction.editReply({
          embeds: [
            {
              author: {
                name: 'Максимальна кількість символів embed.description = 4096',
              },
              color: 0xeb4c34,
            },
          ],
          ephemeral: true,
          components: [],
        }) //Return an error if embed description is too long
    ...
    if (embedFooter && embedFooter.length > 2048)
      return interaction.editReply({
        embeds: [
          {
            author: {
              name: 'Максимальна кількість символів embed.footer = 2048',
            },
            color: 0xeb4c34,
          },
        ],
        ephemeral: true,
        components: [],
      }) //Return an error if embed footer is too long
    if (embedFooter && embedFooterIcon && embedFooterIcon.contentType.startsWith('image')) {
      //Check if footer is specified. If footer icon also was specified, and it's an image, put it as footer icon
    ...
    //If embed is present, attach it to the final message
    ...
    await interaction
      .editReply(message) //Edit pending message with created one
      .catch(async () => {
        return await interaction.editReply({
          embeds: [
            {
              author: {
                name: 'Виникла помилка при формуванні повідомлення. Перевірте правильність даних.',
              },
              color: 0xeb4c34,
            },
          ],
    ...
    collector.on('collect', async () => {
      ...
      if (options.getString('author') === 'server') {
        if (!interaction.guild.members.me.permissionsIn(channel).has(PermissionFlagsBits.ManageWebhooks)) {
          return await interaction.editReply({
            ...
            embeds: [
              {
                author: {
                  name: 'Не маю прав щоб керувати WebHook',
                },
                color: 0xeb4c34,
              },
            ],
        ...
        message.avatarURL = await interaction.guild.iconURL({
          dynamic: true,
        }) //Also set message author avatar (webhook avatar) as server avatar

        await webhook
          .send(message) //Send webhook
          .then(async () => {
              ...
              embeds: [
                {
                  ...
                  color: 0x344feb,
                  description: `Повідомлення було відправлено в канал ${channel} від імені Серверу.`,
                },
              ],
              ephemeral: true,
              components: [],
        ...
        await channel
          .send(message) //Send message to previously specified channel
          .then(async () => {
            //Success message if sent
            await interaction.editReply({
              content: '',
              embeds: [
                {
                  ...
                  color: 0x9f34eb,
                  description: `Повідомлення було відправлено в канал ${channel} від імені Бота.`,
                },
              ],
              ephemeral: true,
              components: [],
    ...
    collector.on('end', async (collected) => {
      if (!collected.first()) {
        interaction.followUp('Дія відмінена від неактивності')
