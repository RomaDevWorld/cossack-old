...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const secRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setLabel(`Закрити голосування`).setCustomId(`cvote`).setStyle(ButtonStyle.Danger))

    const embed = new EmbedBuilder()
      .setAuthor({
        name: `${interaction.options.getString('label').slice(0, 256)}`,
      })
