...
module.exports = async function (type, client, options) {
  ...
  if (type === 'msgDelete') {
    ...
    let embed = new EmbedBuilder()
      .setAuthor({
        name: `Повідомлення видалено | ${options.message.member?.nickname || options.message.author.username}`,
        iconURL: options.message.member.displayAvatarURL({
          dynamic: true,
        }),
      })
      ...
      .addFields(
        {
          name: 'Автор',
          value: `${options.message.author} (${options.message.author.discriminator == '0' ? options.message.author.username : options.message.author.tag})`,
          inline: true,
        },
        {
          name: 'Канал',
          value: `${options.message.channel} (#${options.message.channel.name})`,
          inline: true,
        }
      )
    ...
    if (options.message.attachments.first()) {
      embed.addFields({
        name: 'Вкладені файли',
        value: `${options.message.attachments
          .map((a) => a.url)
          .slice(0, 1024)
          .join(`\n`)}`,
      })
    }
    await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`)) //Creates an embed and send's it to the log channel
  }
  //messageUpdate
    ...
    let embed = new EmbedBuilder()
      .setAuthor({
        name: `Повідомлення відредаговано | ${options.newMessage.member.nickname || options.newMessage.author.username}`,
        iconURL: options.newMessage.member.displayAvatarURL({
          dynamic: true,
        }),
      })
      ...
      .addFields(
        ...
        {
          name: 'Канал',
          value: `${options.newMessage.channel} (#${options.newMessage.channel.name})`,
          inline: true,
        },
        {
          name: 'Раніше:',
          value: `${options.oldMessage.content || `?`}`,
        },
        {
          name: 'Зараз:',
          value: `${options.newMessage.content || `?`}`,
        }
      )
    ...
    if (options.newMessage.attachments.first() || options.oldMessage.attachments.first()) {
      ...
      embed.addFields({
        name: 'Вкладені файли',
        value: `**Раніше:**\n${options.oldMessage.attachments
          .map((i) => i.url)
          .slice(0, 1024)
          .join(`\n`)}\n\n**Зараз:**\n${options.newMessage.attachments
          .map((i) => i.url)
          .slice(0, 1024)
          .join(`\n`)}`,
      })
    ...
    await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`))
  }
  //guildMemberUpdate
    ...
    if (options.oldMember.nickname !== options.newMember.nickname) {
      ...
      let embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: 'Раніше',
            value: `${options.oldMember.nickname}`,
            inline: true,
          },
          {
            name: 'Зараз',
            value: `${options.newMember.nickname}`,
            inline: true,
          }
        )
        ...
        .setColor('Blue')
        .setTimestamp()
      let log = await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`)) //Create an embed then send it to the log
    ...
    let embed = new EmbedBuilder()
      .setAuthor({
        name: `Ролі змінено! | ${options.newMember.nickname || options.newMember.user.username}`,
        iconURL: options.newMember.displayAvatarURL({ dynamic: true }),
      })
      .setColor('Yellow')
      .setTimestamp()
      .setFooter({ text: `USID: ${options.newMember.id}` })
    ...
    if (embed.data.fields) {
      let log = await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`)) //If at least one of the fields was added - send embed to the log
      const audit = await fetchLog(channel.guild, AuditLogEvent.MemberUpdate)
      if (!audit || audit.target.id !== options.newMember.id) return

      embed.addFields({ name: 'Модератор', value: `${audit.executor}` })
      log.edit({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`))
    }

    if (options.oldMember.communicationDisabledUntil && !options.newMember.communicationDisabledUntil) {
      const embed = new EmbedBuilder()
        .setAuthor({
          name: `Учасник обмежений | ${options.newMember.nickname || options.newMember.user.username}`,
          iconURL: options.newMember.displayAvatarURL({
            dynamic: true,
          }),
        })
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: 'Час блокування',
            value: `${moment(time).fromNow()} (${moment(time).format('L')})`,
          }
        )
      let log = await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`)) //Create an embed then send it to the log
    ...
    let embed = new EmbedBuilder()
      ...
      .setColor('Orange')
      .setTimestamp()
    let log = await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`))
  ...
  //guildMemberAdd
    ...
    let embed = new EmbedBuilder()
      ...
      .setColor('Green')
      .setTimestamp()
    const msg = await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`))
    ...
    embed.addFields({
      name: 'Запрошення',
      value: `**Код:** ${invite.code}\n**Запросив:** ${inviter || `<@${invite.inviterId}>`}\n**Використано: ${invite.uses ?? '/' + invite.maxUses}**`,
    })

    await msg.edit({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`))
  }
  //guildBanAdd
  else if (type === 'banAdd') {
  ...
  //guildBanRemove
    ...
    let embed = new EmbedBuilder()
      .setAuthor({
        name: `${options.newVoiceState.member.user.discriminator ? options.newVoiceState.member.user.username : options.newVoiceState.member.user.tag}`,
        iconURL: options.newVoiceState.member.user.displayAvatarURL({
          dynamic: true,
        }),
      })
      .setDescription(`**Приєднався до #${options.newVoiceState.channel.name}**`)
      .setFooter({ text: `USID: ${options.newVoiceState.id}` })
    let msg = await channel.send({ embeds: [embed] }).catch((err) => console.error(`An error occurred while trying to send a message: ${err.name}: ${err.message}`))
    ...
    if (!msg) return
    msg.message.embeds[0].data.description = (msg.message.embeds[0].data.description + `\n**Від'єднався, сесія тривала ${new Date(Date.now() - msg.time).toISOString().slice(11, 19)}**`).slice(0, 4000)
...
async function getlog(guild, bool) {
  if (!guild) return
  if (bool === true) {
    //For some reason i've added a boolean?
...
async function isOn(guild, type) {
  ...
  if (typedb && typedb.includes(type)) {
    //If db array has that type
    return true
  } else {
    //Nah
    return false
