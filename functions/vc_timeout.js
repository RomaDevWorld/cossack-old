...
module.exports = async function (newVoiceState, oldVoiceState, client) {
  let privates = client.privates //Requires a privates from client
  for (let i in privates) {
    //Loops through every private
    if (oldVoiceState.channel && oldVoiceState.channel.id === privates[i]) {
      //If user left someone's private channel
      if (oldVoiceState.channel.members.size === 0) {
        //Checks user's joined to this channel, if it's 0
      ...
      if (timeouts[privates[i]]) {
        //If there's a timeout for this channel
